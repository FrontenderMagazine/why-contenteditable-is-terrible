...
[ContentEditable][2] это нативный способ для насыщенного редактирования текста
...
с помощью HTML. Все страницы можно представить как дерево элементов с текстовыми
...
Движок рендера браузера переносит страницу из пространства DOM в Видимое пространство.
...
мы имеем в виду, что оно сохраняет все операции редактирования<a href="#note-1" id="ref-1" class="reference">1</a>.
Если быть точнее, то Рендер четкий, если
...
страницы выглядят одинаково, и мы применяем к ним одинаковые операции редактирования,
то и результат будет одинаковый <a href="#note-1" id="ref-1" class="reference">1</a>.

Я был удивлен, как много “WYSIWYG” редакторов в веб не соответствуют этому критерию.
...
Существует множество способов добиться отображения последнего слова полужирным курсивом <a href="#note-2" id="ref-2" class="reference">2</a>.
...
ведут себя совершенно по-разному (хотя и выглядят идентично). Это не только сводит 
...
Достаточно просто убедиться, что одно выделение может иметь множество соответствующих 
...
перед тегом `strong`, между тегами `strong` и `em` и после тега `em`. Если вы 
...
Когда мы разрабатываем команды для редактора мы хотим, чтобы выделение выглядело
одинаково и одинаково себя вело. Но, так как отображение из пространства DOM в 
Видимое пространство нечеткое — это попаболь.
...
Рьюсуке Нива (Ryosuke Niwa) написал [чудесный пост в блоге WebKit][5] о том, 
как они избавлялись от apple-style-span. Если вы его прочтете, то многие 
затронутые в нем проблемы покажутся вам знакомыми. ContentEditable в WebKit
...
контентом вставленным из буфера, который, в нашем редакторе вообще невозможно 
получить.
...
Чтобы избежать этого класса ошибок, мы считаем, что контент хорошего WYSIWYG 
...
Голый элемент с атрибутом ContentEditable это плохой WYSIWYG-редактор, так как 
...
Любое выделение в редакторе Medium выражается с помощью двух точек. Каждая точка
...
Внешнее соответствие это когда у нас есть HTML, полученный не с помощью нашего 
...
полужирное и курсивное начертание, за ссылки, и только потом — изображения и 
...
Следующий узел это один из семантических типов параграфа: P, H2, H3, PRE, FIGURE,
BLOCKQUOTE, OL-LI (упорядоченный список элементов), и UL-LI (неупорядоченный 
список элементов).
...
Мы преобразуем разметку так, чтобы якорь содержал тег STRONG.
...
Вполне очевидно, что контент в рамках этих операций четко определен. Они работают
...
и удалить параграф с помощью обычных ContentEditable команд. Это: возврат каретки
...
умолчанию. После завершения события клавиатуры, мы получаем модель параграфа из
...
Но в реальном мире, перерисовывание всего поста после каждого нажатия происходит
...
Medium заменяет весь параграф сразу, вместо того, чтобы изменить его часть. 
...
«[Непонятная фигня][8]». Проблемы дизайна и UX могут быть так же не решаемы, как
